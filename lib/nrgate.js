var doughnutData = {};

function respondCanvas(powerUsed) {

    var doughnutOptions = {
        cutoutPercentage : 80, //The percentage of the chart that we cut out of the middle.
        rotation: (.7*Math.PI),
        circumference: (0.8 *2 * Math.PI),
        animation: {
            animateScale: false,
            animateRotate: false
        }
    };

    doughnutData = [{
        data: [powerUsed, 100-powerUsed],
        backgroundColor: ['#1a433f','#ffffff'],
        borderColor: ['#1a433f','#b4f5e0'],
        borderWidth: [0,0]
    }];
    //Get the context of the Doughnut Chart canvas element we want to select
    var ctx = document.getElementById("powerChart");

    // Create the Doughnut Chart
    var powerChart = new Chart(ctx,{
        type: 'doughnut',
        data: {
            labels: [],
            datasets: doughnutData
        },
        options: doughnutOptions
    });
}

var getSensorData = function () {
    $.ajax({
        url: 'api.php',
        method: 'POST',
        dataType: 'json',
        success: function (sensorData) {
            console.log(sensorData);
            respondCanvas(sensorData.watts);
            console.log(document.getElementById("powerChart").innerHTML);
            document.getElementById("powerlabel").innerHTML = sensorData.watts;
            document.getElementById("temp-label").innerHTML = sensorData.temp;
            document.getElementById("co2-label").innerHTML = sensorData.airquality;
        }
    });
};

$(document).ready(function() {
    setInterval(getSensorData(5),1000);
});